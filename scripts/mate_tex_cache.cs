...
public static class MateTexCache
{
    ...
    static public ConfigEntry<int> _tempCacheCapacity = configFile.Bind("MateTexCache Setting", "TempCacheCapacity", 10, new ConfigDescription("Capacity for Temp Cache (will Destory)", new AcceptableValueRange<int>(0, 100)));
    ...
    public class LRUCache<K, V> : IEnumerable<LRUCacheItem<K, V>> where V : IDisposable
    {
        ...
        public void Remove(K key, bool destory = false)
        {
            if (cacheDict.TryGetValue(key, out var node))
            {
                cacheList.Remove(node);
